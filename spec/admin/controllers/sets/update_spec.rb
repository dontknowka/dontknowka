RSpec.describe Admin::Controllers::Sets::Update, type: :action do
  let(:action) { described_class.new(sets: sets) }
  let(:params) { { 'rack.session' => session, :id => id, :set => set } }
  ...
  context 'when user is logged in' do
    ...
    it 'is successful' do
      expect(sets).to receive(:update).with(id, set)
  ...
  context 'when user is not logged in' do
    ...
    it 'is redirected to login page' do
      expect(sets).not_to receive(:update)
      response = action.call(params)
      expect(response[0]).to eq 302
